import {describe,it,expect} from 'vitest';import {haversineMeters,startOfChinaDay} from '../lib/utils';import {cleanPublicValue} from '../lib/public-config';import {safeUploadPath} from '../lib/uploads';
...
describe('部署配置',()=>{it('清除环境变量外层引号',()=>expect(cleanPublicValue('"amap-key"')).toBe('amap-key'));it('拒绝上传目录穿越',()=>expect(safeUploadPath(['..','secret'])).toBeNull())});

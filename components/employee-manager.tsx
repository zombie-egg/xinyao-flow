...
import { roleText, userStatusText } from "@/lib/chinese-labels";
type Employee = {
  id: string;
  name: string;
  username: string;
...
export function EmployeeManager({
  ...
  async function save(id: string, e: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage(`员工信息已保存，工龄自动计算为 ${body.data.workYears} 年`);
    setEditing(null);
    router.refresh();
  }
  ...
  return (
    <div className="overflow-x-auto rounded-xl border bg-white">
        ...
        <tbody>
          {users.map((x) => (
            <tr key={x.id} className="border-t align-top">
              ...
              <td className="px-4 py-4">{x.jobTitle}</td>
              <td className="px-4 py-4">{x.workYears} 年</td>
              {canEdit && editing === x.id ? (
                <td className="px-4 py-3">
                  ...
                  >
                    <Input
                      ...
                      type="date"
                      defaultValue={dateValue(x.employmentStartDate)}
                      className="w-40"
                      aria-label="入职时间"
                      required
                    />
                ...
                <td className="px-4 py-4">
                  {dateValue(x.employmentStartDate)}
                </td>
              )}
              <td className="px-4 py-4">
                {Number(x.annualLeaveUsed)} / {Number(x.annualLeaveDays)} 天
              </td>
              <td className="px-4 py-4">
                <Badge>{userStatusText[x.status]}</Badge>
              </td>
              <td className="px-4 py-4">
                <Link
                  href={`/employees/${x.id}/attendance`}
      ...
      {message && editing === null && (
        <p className="border-t px-4 py-3 text-sm text-zinc-500">{message}</p>
      )}
    </div>
  );
}

...
export function CustomerFilters({ params, salesUsers }: { params: Record<string, string | undefined>; salesUsers: { id: string; name: string }[] }) {
  ...
  return (
    <div className="mb-5 space-y-3">
      <div className="flex flex-nowrap gap-1.5 overflow-x-auto pb-1">
        {quickItems.map(([value, label]) => <Link key={value} href={`/customers?quick=${value}`} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-medium ${quick === value ? "bg-zinc-950 text-white" : "border bg-white text-zinc-600"}`}>{label}</Link>)}

...
export default async function Statistics({ searchParams }: { searchParams: Promise<{ start?: string; end?: string }> }) {
  ...
  const [rows, paymentAggregate] = await Promise.all([
    db.order.groupBy({ by: ["salesUserId", "historicalSalesName"], where, _count: { id: true }, _sum: { amount: true, paidAmount: true }, orderBy: { _sum: { amount: "desc" } } }),
  ...
  return <>
    ...
    {!rows.length ? <Empty /> : <Card><div className="space-y-5">{rows.map((row, index) => { const amount = Number(row._sum.amount || 0); const max = Math.max(...rows.map((item) => Number(item._sum.amount || 0))); return <div key={`${row.salesUserId}-${row.historicalSalesName || "active"}`}><div className="mb-2 flex justify-between gap-4 text-sm"><span>{index + 1}. {row.historicalSalesName || users.find((user) => user.id === row.salesUserId)?.name}</span><span>{row._count.id} 单 · {money(amount)} · 已回款 {money(Number(row._sum.paidAmount || 0))}</span></div><div className="h-2 rounded-full bg-zinc-100"><div className="h-2 rounded-full bg-zinc-900" style={{ width: `${max ? Math.max(4, amount / max * 100) : 0}%` }} /></div></div>; })}</div></Card>}
  </>;
}

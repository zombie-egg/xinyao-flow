...
import { CustomerReleaseToPool } from "@/components/customer-release-to-pool";
...
export default async function CustomerDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-wrap gap-2">{customer.isPublicPool && hasSalesCapabilities(user.role.code) ? <PublicCustomerClaim customerId={customer.id} salesUsers={salesUsers} /> : canManageCustomer && <><Link href={`/orders/new?customerId=${customer.id}`} className="inline-flex h-10 items-center rounded-lg bg-zinc-950 px-4 text-sm font-medium text-white">为该客户新建订单</Link><CustomerReleaseToPool customerId={customer.id} /></>}<Link href="/customers" className="inline-flex h-10 items-center rounded-lg border px-4 text-sm">返回客户管理</Link></div>
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Card className="xl:col-span-2"><h2 className="font-medium">历史订单</h2>{customer.orders.length ? <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{customer.orders.map((order) => <Link key={order.id} href={`/orders/${order.id}`} className="rounded-xl border p-4 hover:bg-zinc-50"><div className="flex items-center justify-between gap-3"><p className="font-medium">{order.orderNumber || "待审核订单"}</p><Badge>{businessOrderStatus(order)}</Badge></div><p className="mt-2 text-sm">{order.name}</p><p className="mt-2 text-sm text-zinc-500">销售：{order.historicalSalesName || order.salesUser.name} · {money(Number(order.amount))}</p><p className="mt-1 text-sm text-zinc-500">订单成立日期：{order.createdAt.toLocaleDateString("zh-CN", { timeZone: "Asia/Shanghai" })}</p></Link>)}</div> : <Empty text="该客户暂无订单" />}</Card>
      </div>
    </>
  );
}

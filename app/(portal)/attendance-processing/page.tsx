...
export default async function AttendanceProcessing({
  searchParams,
}: {
  searchParams: Promise<{ view?: string; q?: string }>;
}) {
  const u = await requireUser(),
    admin = u.role.code === "ADMIN",
    manager = u.role.code === "SALES_MANAGER" || u.role.code === "TECH_MANAGER";
  if (!admin && !manager) throw new Error("FORBIDDEN");
  const params = await searchParams,
    view = params.view === "processed" ? "processed" : "pending",
    q = params.q?.trim() || "",
    scope = admin
      ? {}
      : { attendance: { user: { departmentId: u.departmentId } } },
    search = q
      ? {
          OR: [
            {
  ...
  const [items, todayRequirement, setting] = await Promise.all([
      db.attendanceException.findMany({
        where,
        include: {
          attendance: { include: { user: { include: { department: true } } } },
          approvals: {
      ...
      admin
        ? db.dailyAttendanceRequirement.findUnique({
  ...
  return (
    <>
      ...
      <div className="mb-5 flex gap-2">
        <Link
          href={`/attendance-processing?view=pending${suffix}`}
          className={`rounded-lg px-4 py-2 text-sm ${view === "pending" ? "bg-zinc-950 text-white" : "border bg-white"}`}
        >
          待处理
        </Link>
        <Link
          href={`/attendance-processing?view=processed${suffix}`}
          className={`rounded-lg px-4 py-2 text-sm ${view === "processed" ? "bg-zinc-950 text-white" : "border bg-white"}`}
        >
          已处理
        </Link>
      </div>
      <SearchForm
        defaultValue={q}
        placeholder="搜索员工、部门或异常原因"
        hidden={{ view }}
        clearHref={`/attendance-processing?view=${view}`}
      />
      {items.length ? (
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className="font-medium">
                    {item.attendance.user.name} · {typeText[item.type]}
                  </p>
                  <p className="mt-1 text-sm text-zinc-500">
                    {item.attendance.user.department?.name || "—"} ·{" "}
                    {item.attendance.date.toLocaleDateString("zh-CN")} · 原因：
                    {item.reason || "员工尚未填写"}
                  </p>
                  <p className="mt-2 text-xs text-zinc-400">
                    {item.approvals.length
                      ? item.approvals
                          .map(
                            (a) =>
                              `${a.approver.name} ${a.result}${a.comment ? `（${a.comment}）` : ""}`,
                          )
                          .join(" → ")
                      : "暂无处理记录"}{" "}

...
export default async function Performance({ searchParams }: { searchParams: Promise<{ start?: string; end?: string }> }) {
  ...
  const rows = await db.order.groupBy({ by: ["salesUserId"], where, _count: { id: true }, _sum: { amount: true, paidAmount: true }, orderBy: { _sum: { amount: "desc" } } });

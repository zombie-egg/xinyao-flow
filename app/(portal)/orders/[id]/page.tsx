      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Card>
          ...
          <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
            {[['业务类型',order.contract.businessType==='PUBLIC_HEALTH'?'公共卫生':'环境监测'],['合同状态',order.contract.signingStatus==='SIGNED'?'已签订':'待签订'],['产品合计',money(Number(order.contract.productTotal))],['合同金额',money(Number(order.contract.amount))],['技术支持费用',money(Number(order.contract.technicalSupportFee))],['外包费用',money(Number(order.contract.outsourcingFee))],['评审费用',money(Number(order.contract.reviewFee))],['其他支出',money(Number(order.contract.otherExpense))],['净签单金额',money(Number(order.contract.netOrderAmount))],['变更净签单金额',order.contract.adjustedNetAmount==null?'—':money(Number(order.contract.adjustedNetAmount))],['签订人',order.contract.signer?.name||'—'],['签订日期',order.contract.contractDate.toLocaleDateString('zh-CN')],['负责人',order.contract.responsibleUser?.name||'—'],['协同人',order.contract.collaborator?.name||'—']].map(([k,v])=><div key={k} className="flex justify-between gap-4"><span className="text-zinc-500">{k}</span><span className="text-right">{v}</span></div>)}
          </div>

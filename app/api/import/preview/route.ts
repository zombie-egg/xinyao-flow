...
export async function POST(req: Request) {
  try {
    ...
    const publicPool = entity === "customers" && String(form.get("publicPool")) === "true";
    ...
    const preview = parsed.rows.slice(0, 100).map((row, index) => {
      ...
      const collaboratorText = publicPool ? "" : rowValue(row, mapping, entity === "customers" ? "collaborators" : "financeCollaborator");
    ...
    const allPeople = publicPool ? [] : [...new Set(parsed.rows.flatMap((row) => [rowValue(row, mapping, entity === "customers" ? "owner" : "responsible"), rowValue(row, mapping, entity === "customers" ? "collaborators" : "financeCollaborator")].flatMap(splitNames)))];
    return ok({ token: null, filename: file.name, sheetName: parsed.sheetName, totalRows: parsed.rows.length, headers: parsed.headers, targetFields: importFields[entity], mapping, ai, publicPool, salesUsers: sales, unmatchedPeople: allPeople.filter((name) => !matchName(name)), preview, importEnabled: false, message: publicPool ? "已按整份公海客户审核：忽略原负责人和协同人；当前仅预览，不写入数据库。" : "当前仅生成审核预览；确认人员全部注册后再执行正式导入。" });
  } catch (error) {
